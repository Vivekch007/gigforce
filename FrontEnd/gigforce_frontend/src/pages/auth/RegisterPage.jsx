...
// Mirrors com.gigforce.identity.enums.UserRole. ADMIN is listed per spec but the
// backend (AuthServiceImpl.register) rejects ADMIN self-registration with a 400 —
// left enabled so that rejection surfaces as a normal inline error message.
const ROLE_OPTIONS = [
...
// Roles for which AuthServiceImpl.register requires a non-blank orgUnitId.
const ORG_UNIT_REQUIRED_ROLES = ['HIRING_MANAGER', 'FINANCE', 'VENDOR'];
        ...
        <Form.Group className="mb-3" controlId="registerRole">
          ...
          {form.role === 'ADMIN' && (
            <Form.Text className="text-warning d-block mt-1">
              Admin accounts are provisioned by system administrators and cannot self-register.
            </Form.Text>
          )}
        </Form.Group>
