...
function Timesheets() {
  ...
  const handleSaveDraft = async () => {
    ...
    setError('');
    try {
      // Filter out Sat/Sun since backend doesn't pre-generate skeleton for weekend & throws errors
      const editLines = linesData
        .filter((l) => !l.isWeekend && l.isPreGenerated)
        .map((l) => ({
          workDate: l.workDate,
            ...
            <Table responsive className="align-middle mb-0">
              ...
              <tbody>
                {linesData.map((line, idx) => {
                  const isEditable = !line.isWeekend && line.isPreGenerated && (timesheet.status === 'DRAFT' || timesheet.status === 'REJECTED');
                  return (
                    <tr key={idx} className={line.isWeekend ? 'bg-light text-muted' : ''}>
                      ...
                      <td className="p-3">
                        <Form.Control
                          ...
                          disabled={!isEditable}
                          placeholder={line.isWeekend ? 'Weekend - logging locked' : 'Describe your activities...'}
                          className="grid-input"
                        />
                      </td>
                    </tr>
