...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentResponseDTO createPayment(PaymentCreateRequestDTO request) {
        ...
        List<Timesheet> timesheets = timesheetRepository.findByInvoiceId(request.getInvoiceId());
        for(Timesheet timesheet : timesheets) {
            if(timesheet.getStatus() != TimesheetStatus.APPROVED) {
                throw new BusinessValidationException("Cannot process payment against invoice with unapproved timesheets. Timesheet ID: " + timesheet.getId() + " is in status " + timesheet.getStatus());
            }
            if(timesheet.getPayrollStatus() == PayrollStatus.PROCESSING) {
                throw new BusinessValidationException("Cannot process payment against invoice with timesheets already in PROCESSING payroll status. Timesheet ID: " + timesheet.getId());
            }
        }
        ...
        Payment payment = Payment.builder()
                ...
                .build();
        for(Timesheet timesheet : timesheets) {
            timesheet.setPayrollStatus(PayrollStatus.PROCESSING);
        }
    ...
    public PaymentResponseDTO processPayment(String id) {
        ...
        for(Timesheet timesheet : timesheets) {
            timesheet.setPayrollStatus(PayrollStatus.PROCESSED);
        }
        timesheetRepository.saveAll(timesheets);
