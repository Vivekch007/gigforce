...
class PaymentServiceImplTest {
    ...
    private Payment payment(String id, ContractorInvoice inv, PaymentStatus status) {
        Payment p = Payment.builder()
                ...
                .status(status).paymentReference("UTR-1").transactionId("TXN-1").build();
        p.setId(id);
        return p;
    }

    private PaymentRequestDTO req(BigDecimal amount, LocalDate date, String ref, String txn, String mode) {
        return PaymentRequestDTO.builder()
                .invoiceId("inv1").paidAmount(amount).paymentDate(date)
                .paymentMode(mode).paymentReference(ref).transactionId(txn).build();
    }
    ...
    @Test
    void createPayment_missingTransactionId_throws() {
        ContractorInvoice inv = invoice("inv1", InvoiceStatus.APPROVED);
        when(currentUserContext.getCurrentUser()).thenReturn(finance);
        when(contractorInvoiceRepository.findById("inv1")).thenReturn(Optional.of(inv));
        assertThrows(BusinessValidationException.class,
                () -> service.createPayment(req(TOTAL, LocalDate.now(), "UTR-9", "  ", "BANK_TRANSFER")));
    }
    ...
    @Test
    void createPayment_missingReference_throws() {
        ContractorInvoice inv = invoice("inv1", InvoiceStatus.APPROVED);
        when(currentUserContext.getCurrentUser()).thenReturn(finance);
        when(contractorInvoiceRepository.findById("inv1")).thenReturn(Optional.of(inv));
        assertThrows(BusinessValidationException.class,
                () -> service.createPayment(req(TOTAL, LocalDate.now(), "  ", "TXN-9", "BANK_TRANSFER")));
    }
    ...
    void processPayment_pendingToProcessed_marksInvoicePaid() {
        ...
        assertEquals(pay.getPaymentReference(), inv.getPaymentReference());
